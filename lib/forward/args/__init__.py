# (c) 2016, Leann Mak <leannmak@139.com>
#
# This file is part of Forward.
#
# Note: Do not add any code to this file.  The forward module may be
# a namespace package when using Forward. Anything in this file may not be
# available if one of the other packages in the namespace is loaded first.
#
# This is for backwards compat.  Code should be ported to get these from
# forward.release instead of from here.
